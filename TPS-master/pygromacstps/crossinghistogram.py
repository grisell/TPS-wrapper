...
handler = logging.FileHandler("divtest.txt")
...
class generichistogram(object):
    ...
    def __init__(self,histosize=2000,histomin=-4.0,histomax=4.0,forward=True):
        ...
        self.comhisto = np.zeros(histosize) #GDL: added a histogram for commitor
    ...
    def addPointToHisto(self,value):
        ...
        """
        #hbox = int(round(self._getBox(value)))-1
        hbox = int(round(self._getBox(value)))
        self.histo[hbox] += 1.0
        self.norm += 1.0

    
    def addweightedPointToHisto(self,value,weight): #GDL:Adding weighted point to histogram
        """
        Add a single point to the histogram
        """
        hbox = int(round(self._getBox(value)))
    ...
    def addweightedPointTocommittorHisto(self,value,weight): #GDL:Adding weighted point to commitor histogram
        """
        Add a single point to the histogram
        """
        hbox = int(round(self._getBox(value)))
...
#            of.write("%.18f %.18f\n" % (self._getValue(i),self.histo[i]/self.norm))
    ...
    def outputcommitorgHisto(self,filename): #GDL: output commitor histogram
        """
        Print the histogram
        """
        of = open(filename,"w")
...
#            of.write("%.18f %.18f\n" % (self._getValue(i),self.histo[i]/self.norm))
    ...
    def outputfreeenergyHisto(self,filename,unit): #GDL: output free energy histogram
        """
        Print the histogram
        """
        max=-1.0

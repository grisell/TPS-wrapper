...
dirpath = os.path.join(os.getcwd(),'tis')
...
class rpeData(object):
    ...
    def readpathweights(self,ninter,forward): 
        ...
        PAnorm= [0]*ninter
        weight= [0]*ninter
        filename = forward and os.path.join(dirpath,"selfconsistent","fpathweights.txt") or os.path.join(dirpath,"selfconsistent","bpathweights.txt")       
        c = 0
    ...
    def readcacb(self): 
        ...
        """
        filename = os.path.join(dirpath,"selfconsistent_fe","cacb.dat")       
        c = 0
        for line in open( filename , 'r' ):

...
LARGENUMBER=1000000
temperature=1370.0
beta=1/(temperature*0.000086112) # unit=K/eV
...
class rpeData(object):
    ...
    def readcv2FullTrajectory(self,filename,pathnumber):
        ...
        for line in open(os.path.join(filename),"r"):
            raw = line.split()
            traj.append([int(float(raw[0])),float(raw[1]),float(raw[2]), float(raw[3]),float(raw[4]),float(raw[5]),float(raw[6]),float(raw[7]),float(raw[8]),float(raw[9]),float(raw[10]), float(raw[11]),int(float(raw[12]))])
            if int(float(raw[12])) == 0:
    ...
    def voronoi(self,nstrpoints, stingvar,cv1,cv2, weight, interface,pointtoB):
...
#***********************************************MLE from RPE  using  non-linear reaction coordinate (string) **********************************************************************************
if __name__=="__main__":
    ...
    for i in range(ntot):
        ...
        k=random.randint(0,nstrpoints-1)

        """
        Select string point to modify 
        """
        for i in range(ncvs):
            string_old_i=string[k][0]
            string_old_i=string[k][1]
        
        for i in range(nstrpoints):
            for j in range(ncvs):
                string_old[i][j]=string[i][j]
        ...
        print dx, dy
        for i in range(ncvs):
            string[k][i]= string_old_i +  dx
            #string[k][1]= string_old_i +  dy
        #else:
        ...
        while (cdata.check_string_consistency(nstrpoints, string, deviation)==False):
            ...
            #cdata.check_string_consistency(nstrpoints, string, deviation)
            #print string[k][0], string[k][1]
        print "moving string point:", k, "x=", string[k][0], "y=", string[k][1]
    ...
    #            integral[i/2] += 1
        ...
        for dirnumber in range(start,stop): 
            ...
            for i in cdata.kernels.kernelPaths:   
                ...
                print "Getting pB and pA histograms from RPE"
                for point in cdata.paths[i].lastAcceptedFullTrajectory[0]:
                    for point2 in cdata.cv2paths[i].lastAcceptedFullTrajectory[0]:
                        #if point > cdata.interfaces.interfaces[0][0]:
                        #print point[1], point2[CV2COLUMN]
                        cdata.voronoi(nstrpoints, string,point[1],point2[CV2COLUMN], weight,i,pointtoB)
        ...
        reject=0
        rn= random.random()
        print rn, aux, exp(aux)
        if (rn > exp(aux)):
            print "value of likelihood, lnL=", lnL, "aux=", aux
            for i in range(ncvs):
                string[k][i]= string_old_i
            ...
            for i in range(nstrpoints):
                for j in range(ncvs):
                    string[i][j]= string_old[i][j]
                #string[i][1]= string_old[i][1]
            print "restored lnL =", lnL, "oldlnL=", lnLn_old
        else:

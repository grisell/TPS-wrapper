...
class THisto(object):
    def __init__(self,index,interface,forward):
        ...
        ifilename = self.forward and "forward/crossinghisto.%02d.dat" % (index*2,) or "backward/crossinghisto.%02d.dat" % (index*2,)
...
#    
    def readHisto(self):
        filename = self.forward and os.path.join(ospath,"forward/crossinghisto.%d.txt" % (self.index*2,)) or os.path.join(ospath,"backward/crossinghisto.%d.txt" % (self.index*2,))       
        c = 0

...
#        self.distparser = parser.gdistparser()
        ...
        self.freeenergyhistosStableA = [] #DS
        self.freeenergyhistosStableB = [] #DS
        ...
        for i in range(self.interfaces.ninterfaces[0]):
            ...
            self.crossinghistos.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=True))
            self.freeenergyhistos.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=True))
            self.boundaryhistos.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=True))      #DS init the boudaryhistogram AB
            self.npaths += 1
        self.freeenergyhistosStableA.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=True)) #DS FE histogram for stable stateA
        ...
        for i in range(self.interfaces.ninterfaces[1]):
            ...
            self.crossinghistos.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=False))
            self.freeenergyhistos.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=False))
            self.boundaryhistos.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=False))     #DS init the boudaryhistogram BA
            self.npaths += 1        
        self.freeenergyhistosStableB.append(crossinghistogram.generichistogram(histosize=1200,histomin=-0.2,histomax=1.0,forward=False)) #DS FE histogram for stable stateB
...
#DS construct the FE histogram for stable statesA
        filename = os.path.join(basedir,"tis/la/stateA",dirstring,"trajectoryZ.00."+dirstring+".acc")
        cdata.readFullTrajectory(filename, 0)
        cdata.crossinghistos[0].addRangeToHistoFromInterface(cdata.paths[0].getMaxTrajectoryValue(),cdata.paths[0].interface)
        below = cdata.paths[0].getPointsBelowInterface() 
        for point in below:
            cdata.freeenergyhistosStableA[0].addPointToHisto(point)

#DS construct the FE histogram for stable statesB
        last = (len(cdata.kernels.kernelPaths)-1)*2
        filename = os.path.join(basedir,"tis/la/stateB",dirstring,"trajectoryZ.00."+dirstring+".acc")
        cdata.readFullTrajectory(filename, last)
        cdata.crossinghistos[0].addRangeToHistoFromInterface(cdata.paths[last].getMaxTrajectoryValue(),cdata.paths[last].interface)
        below = cdata.paths[last].getPointsBelowInterface()
        for point in below:
            cdata.freeenergyhistosStableB[0].addPointToHisto(point)
#---------------------------------------------------------------------------------------------------------#DS
...
#DS print the FE histogram for stable stateA
    filename = os.path.join(basedir,"tis/la/stateA","freeenergyhistoStableA.txt")
    cdata.freeenergyhistosStableA[0].outputCrossingHisto(filename)

#DS print the FE histogram for stable stateB
    filename = os.path.join(basedir,"tis/la/stateB","freeenergyhistoStableB.txt")
    cdata.freeenergyhistosStableB[0].outputCrossingHisto(filename)

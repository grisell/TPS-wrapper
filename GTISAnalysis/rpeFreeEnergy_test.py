...
class rpeData(object):
    ...
    def outputcommitorgHisto(self,filename):
        ...
        for i in range(self.histosize):
            ...
            #else:
            #    of.write("%.18f %.18f\n" % (self.getValue(i), self.comhisto[i]))   
        of.close()
    ...
    for dirnumber in range(start,stop): 
        ...
        for i in cdata.kernels.kernelPaths:   
           ...
           # scalefactor=cdata.paths[i].getscalenumbersinterfaces(numpaths[i/2],numinterpaths)
            ...
            if (FILETYPE==1):
                if(len(cdata.paths[i].lastAcceptedFullTrajectory[0]) != len(cdata.cv2paths[i].lastAcceptedFullTrajectory[0])):
                    print "Error:CV file lenghts do not match"
			...
			if (point[1] >= STATEA and  point[1] <= STATEB):
				cdata.addweightedPointToHisto(point2[CVCOLUMN],weight)
                        	#cdata.addweightedPointToHisto(point2[CVCOLUMN]*point[1],weight)                 
                        	if  cdata.paths[i].checifendinstateB(cdata.orderparameters.op):
					cdata.addweightedPointTocommittorHisto(point2[CVCOLUMN],weight)
                                	#cdata.addweightedPointTocommittorHisto(point2[CVCOLUMN]*point[1],weight)
                        #else:
                        #    if point[1]  <  cdata.paths[i].interface:
                        #        cdata.addweightedPointToHisto(point2[CVCOLUMN],weight)
                        #        if  cdata.paths[i].checifendinstateB(cdata.orderparameters.op):
            ...
            if (FILETYPE==2):
                if(len(cdata.paths[i].lastAcceptedFullTrajectory[0]) != len(cdata.ns4paths[i].lastAcceptedFullTrajectory[0])):
                    print "Error:CV file lenghts do not match"

                for (point, point2) in zip(cdata.paths[i].lastAcceptedFullTrajectory[0], cdata.ns4paths[i].lastAcceptedFullTrajectory[0]):
                    if (point[2]==flagshoot):
                        #if cdata.paths[i].forward:
                        #    if point[1]  >  cdata.paths[i].interface:
                        if point[1]  <= 700.0:

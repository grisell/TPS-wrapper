...
FILETYPE=1  #type 0= 3 columns; type 1 = 14 columns; type 2= 8 columns
CVCOLUMN=3
LARGENUMBER=1000000
temperature=1370.0
beta=1.0
...
class rpeData(object):
    ...
    def rcalpha1D(self,ssA, ssB,cv1):
                                
        x=cv1
        #y=cv2
        
        if (ssA > ssB):
            rc=1.0-((x-ssB)/fabs(ssB-ssA))
        else:
            if (ssB > ssA):
                rc= (x-ssA)/fabs(ssB-ssA)
    ...
    def outputstring1D(self,filename,nstrpoints,stringvar,stringrc):
        of = open(filename,"w")
        for i in range(nstrpoints):
            of.write("%d %.18f %.18f \n" % (i, stringvar[i][0],stringrc[i]))
...
if __name__=="__main__":
    ...
    lnLfilename="avlnL.dat"
    lnLrcfilename="avlnLstring.dat"
    ...
    avstring= np.zeros(shape=(nstrpoints))
    ...
    for m in range(ntot):
        ...
        dx=(random.uniform(0.0,1.0)-0.5)*dispx
        ...
        print "moving string point:", kpoint, "x=", string[kpoint][0]
     ...
     #            integral[i/2] += 1
        ...
        for dirnumber in range(start,stop): 
            ...
            for i in cdata.kernels.kernelPaths:   
                ...
                if (FILETYPE==0):
                    ...
                    if (FILETYPE==2):
                        newfilename= os.path.join(cdata.ns4paths[i].nfsladir,dirstring,TRAJECTORYFILE+dirstring+".dat")
                ...
                if (FILETYPE==0):
                    for (tispoint, point) in zip(cdata.paths[i].lastAcceptedFullTrajectory[0], cdata.newpaths[i].lastAcceptedFullTrajectory[0]):
                    
                        
                        
                        if (tispoint[2]==flagshoot): #discard double shooting point
                        
                            if (tispoint[1]>=ss1 and tispoint[1]<=ss2):

                                cdata.voronoi(nstrpoints, string,point[CVCOLUMN], weight,i,pointtoB)
                            
                                #rc=cdata.rcalpha1D(STATEA,STATEB,point[CVCOLUMN])
                                #rc=(rc*20.0)-10.0
                                
                                #if(pointtoB==1):
                                 #   lnpB+=weight*log(0.5*(1.0+tanh(rc)))
                                #else:
                                #    if(pointtoB==0):
                                #        lnpA+=weight*log(0.5*(1.0-tanh(rc)))
                    ...
                    if (FILETYPE==1):
                        for (tispoint, point) in zip(cdata.paths[i].lastAcceptedFullTrajectory[0], cdata.cv2paths[i].lastAcceptedFullTrajectory[0]):
                    ...
                    if (FILETYPE==2):
                        for (tispoint, point) in zip(cdata.paths[i].lastAcceptedFullTrajectory[0], cdata.ns4paths[i].lastAcceptedFullTrajectory[0]):
                            ...
                            if (tispoint[2]==flagshoot): #discard double shooting point
                                if (tispoint[1]>=ss1 and tispoint[1]<=ss2):
                                    ...
                                    #if(pointtoB==1):
                                    #    lnpB+=weight*log(0.5*(1.0+tanh(0.375*(rc-0.367))))
                                    #else:
                                    #    if(pointtoB==0):
                                    #        lnpA+=weight*log(0.5*(1.0-tanh(0.375*(rc-0.367))))
        ...
        #lnLrcalpha=lnpB+lnpA
        #print "lnL(rc)=", lnLrcalpha
        ...
        #    lnLfit += cdata.PBhisto[i]*log(0.5*(1.0+tanh(0.54*(x-1.36)))) + cdata.PAhisto[i]* log(0.5*(1.0-tanh(0.54*(x-1.36))))
        #print "lnLfit=", lnLfit
        ...
        print "rn2=", rn2, "aux2=",aux2, "exp(aux2)=",exp(aux2)
            
        if (rn2 > exp(aux2)):
            print "value of likelihood, lnLrc=", lnLrc, "aux=", aux2
            ...
            print "restored lnL =", lnLrc, "oldlnL=", lnLrc_old
        else:
	    accept+=1
            print "string move accepted  ln Lrc =", lnLrc, "oldlnLrc = ", lnLrc_old
    ...
    """
    for i in range(nstrpoints):
        xrc=slope*(-0.5 +  (i +0.5)/nstrpoints)
        avstring[i]=  avstring[i]/nstringsteps
        avstringrc[i]=  avstringrc[i]/nstringsteps
        print i, xrc, string[i][0],stringrc[i], avstring[i], avstringrc[i]
    ...
    cdata.outputstring1D(stringfile,nstrpoints,string,stringrc)

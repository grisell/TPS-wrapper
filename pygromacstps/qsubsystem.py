...
class qsubsystem(object):
    ...
    def writeKernelQsubFile(self,basedir,kernel,pythonpath,mode,dirstring,reverse=False,method="tis",qsubs="Standard",walltime="100:00:00",queuename="serial",tispath="",wrappath=""):
	    ...
	    wf.write("pwd\n")
            ...
            pythoncommand = os.path.join(pythonpath,"python")
            wf.write("#!/bin/bash\n")
            wf.write("#SBATCH --job-name=TIS-LJ" + qn)
            #wf.write("#SBATCH --time=1:59:00\n")
            wf.write("#SBATCH --partition " + "shorttime" + "\n")
            wf.write("#SBATCH --ntasks=" + "40" + "\n")
...
#            wf.write("module unload 64/intel/12.0.0.08\n")
            ...
            if queuename == 'serial-1':
                queue = 'serial.q'
                nslots = 1
            elif queuename == 'serial-2':
                queue = 'serial.q'
                nslots = 2
            elif queuename == 'serial-3':
                queue = 'serial.q'
                nslots = 3
            elif queuename == 'serial-4':
                queue = 'serial.q'
                nslots = 4
            elif queuename == 'serial-5':
                queue = 'serial.q'
                nslots = 5
            elif queuename == 'serial-6':
                queue = 'serial.q'
                nslots = 6
            elif queuename == 'serial-7':
                queue = 'serial.q'
                nslots = 7
            elif queuename == 'serial-8':
                queue = 'serial.q'
                nslots = 8
            else:
